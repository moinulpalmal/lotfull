                ...
                <div class="row breadcrumbs-top">
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active"><a href="{{route('settings.vendor.setup')}}">Vendor List</a>
                            </li>
                        </ol>
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Vendor Insert/Update Form</h4>
                                <a class="heading-elements-toggle"><i class="fa fa-ellipsis-v font-medium-3"></i></a>
                                <div class="heading-elements">
                                    <ul class="list-inline mb-0">
                                        <li><a data-action="collapse"><i class="feather icon-minus"></i></a></li>
                                        <li><a data-action="reload" onclick="clearForm('WorkExperienceForm'); changeButtonText(' Save', 'submit_button_department', 3); /*clearCheckBox(); *//*resetCkeditor('Description');*/"><i class="feather icon-rotate-cw"></i></a></li>
                                            ...
                                            <div class="row">
                                                <div class="col-md-4 no-padding">
                                                    <div class="form-group">
                                                        <label for="Degree" class="">Vendor Name</label>
                                                        <input type="text" id="Degree" maxlength="180" class="form-control" placeholder="Enter vendor Name" name="name" required>
                                                    </div>
                                                </div>
                                                <div class="col-md-8 no-padding">
                                                    <div class="form-group">
                                                        <label for="ContactInfo" class="">Contact Info</label>
                                                        <input type="text" id="ContactInfo" maxlength="250" class="form-control" placeholder="Enter contact info" name="contact_info">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    <script>
        ...
        function loadDataTable() {
            ...
            dataTable = $('.social-media').DataTable({
                ajax: {
                    url: "/lotfull/public/api/settings/vendor/setup/not-deleted-list",
                    dataSrc: ""
                },
                columns: [
                    {
                    ...
                    {
                        render: function(data, type, api_item){
                            if(api_item.contact_info === null){
                                return "<p class = 'text-left'></p>";
                            }else{
                                return "<p class = 'text-left'>"+ api_item.contact_info +"</p>";
                            }
                        }
                    },
                    {
        ...
        $(function(){
            ...
            $('#WorkExperienceForm').submit(function(e){
                ...
                var url = '{{ route('settings.vendor.setup.save') }}';
                //console.log(data);
                $.ajax({
                    url: url,
                    method:'POST',
        ...
        $('#social-media-table').on('click',".DeleteWorkExp", function(){
            ...
            var url = '{{ route('settings.vendor.setup.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#social-media-table').on('click',".DeActivateWorkExp", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('settings.vendor.setup.de-activate') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#social-media-table').on('click',".ActivateWorkExp", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('settings.vendor.setup.activate') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#social-media-table').on('click',".EditWorkExp", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('settings.vendor.setup.edit') }}';
            $.ajax({
                url: url,
                method:'POST',
                data:{id: id, _token: '{{csrf_token()}}'},
