                ...
                <div class="row breadcrumbs-top">
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active"><a href="{{route('settings.stock-threshold.setup')}}">Strong Thresh List</a>
                            </li>
                        </ol>
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Stock Thresh Insert/Update Form</h4>
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label for="Degree" class="">Stock Threshold Status</label>
                                                        <input type="text" id="Degree" maxlength="200" class="form-control" placeholder="Enter Stock threshold status" name="stock_threshold_status" required>
                                                    </div>
                                                </div>
                                                <div class="col-md-2">
                                                    <div class="form-group">
                                                        <label for="MinDay" class="">Min Day</label>
                                                        <input type="number" step="any" id="MinDay" class="form-control" name="min_day" required>
                                                    </div>
                                                </div>
                                                <div class="col-md-2">
                                                    <div class="form-group">
                                                        <label for="MaxDay" class="">Max Day</label>
                                                        <input type="number" step="any" id="MaxDay" class="form-control" name="max_day" required>
                                                    </div>
                                                </div>
                                                <div class="col-md-2">
                                                    <div class="form-group">
                                                        <label for="ColorCode" class="">Color Code</label>
                                                        <input type="color" id="ColorCode" class="form-control" value="#ff0000" name="color_code" required>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
            ...
            <section id="configuration">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Strong Thresh List</h4>
                                <a class="heading-elements-toggle"><i class="fa fa-ellipsis-v font-medium-3"></i></a>
                                <div class="heading-elements">
                                    <ul class="list-inline mb-0">
                            ...
                            <div class="card-content collapse show">
                                <div class="card-body card-dashboard">
                                    <table id="social-media-table" class="table table-striped table-bordered table-condensed social-media table-info">
                                        <thead>
                                            <tr>
                                                <th class="text-center">Threshold Status</th>
                                                <th class="text-center">Min Day</th>
                                                <th class="text-center">Max Day</th>
                                        ...
                                        @if(!empty($departments))
                                            @foreach($departments as $media)
                                                <tr @if($media->status == 'I') class="bg-warning" @endif>
                                                    ...
                                                    <td class="text-center text-bold-700" style="background-color: {{$media->color_code}}">
                                                        {{$media->color_code}}
                                                    </td>
                                                    <td class="text-center">
...
@section('pageScripts')
    ...
    <script>
        ...
        $(function(){
            ...
            $('#WorkExperienceForm').submit(function(e){
               ...
               /* for ( instance in CKEDITOR.instances ) {
                ...
                var url = '{{ route('settings.stock-threshold.setup.save') }}';
               // console.log(data);
              //  return;
                $.ajax({
                    url: url,
        ...
        $('#social-media-table').on('click',".DeleteWorkExp", function(){
            ...
            var url = '{{ route('settings.stock-threshold.setup.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#social-media-table').on('click',".DeActivateWorkExp", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('settings.stock-threshold.setup.de-activate') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#social-media-table').on('click',".ActivateWorkExp", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('settings.stock-threshold.setup.activate') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#social-media-table').on('click',".EditWorkExp", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('settings.stock-threshold.setup.edit') }}';
            $.ajax({
                url: url,
                method:'POST',
                data:{id: id, _token: '{{csrf_token()}}'},
                success:function(data){
                    $('input[name=stock_threshold_status]').val(data.stock_threshold_status);
                    $('input[name=color_code]').val(data.color_code);
                    $('input[name=min_day]').val(data.min_day);
                    $('input[name=max_day]').val(data.max_day);
