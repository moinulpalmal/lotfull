                ...
                <div class="row breadcrumbs-top">
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active"><a href="{{route('admin.location.setup')}}">Location List</a>
                            </li>
                        </ol>
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Location Insert/Update Form</h4>
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label for="Degree" class="">Location Name</label>
                                                        <input type="text" id="Degree" maxlength="250" class="form-control" placeholder="Enter Location Name" name="name" required>
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label for="ShortName" class="">Location Short Name</label>
                                                        <input type="text" id="ShortName" maxlength="50" class="form-control" placeholder="Enter Location Short Name" name="short_name" required>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
@section('pageScripts')
    ...
    <script>
        ...
        $(function(){
            ...
            $('#WorkExperienceForm').submit(function(e){
               ...
               /* for ( instance in CKEDITOR.instances ) {
                ...
                var url = '{{ route('admin.location.setup.save') }}';
                //console.log(data);
                $.ajax({
                    url: url,
                    method:'POST',
        ...
        $('#social-media-table').on('click',".DeleteWorkExp", function(){
            ...
            var url = '{{ route('admin.location.setup.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#social-media-table').on('click',".DeActivateWorkExp", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('admin.location.setup.de-activate') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#social-media-table').on('click',".ActivateWorkExp", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('admin.location.setup.activate') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#social-media-table').on('click',".EditWorkExp", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('admin.location.setup.edit') }}';
            $.ajax({
                url: url,
                method:'POST',
                data:{id: id, _token: '{{csrf_token()}}'},
